...
typedef struct wav_file_s{
    wav_file_t *next;
    char file_path[256]; //funktioniert das so oder muss ich den speicher mallocen ?
} wav_file_t;

wav_file_t *current_wav_file = NULL;
wav_file_t *start_wav_file = NULL;
unsigned int wav_file_counter = 0; //singlethread access
...
#pragma pack(pop) //TODO doppelt gepackt notwendig/sinnvoll ?
